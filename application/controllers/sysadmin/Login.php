...
class Login extends CI_Controller {
	...
	public function index($page='sysadmin/login')
	{
		$general_settings=$this->Web_model->general_settings();
		$data['general_settings']=$general_settings;
		$data['Site_name']=$general_settings->row()->Site_name;
		$data['Page_name']='Admin Signin';
			...
			if ($email_exists->num_rows()>0) 
			{
				...
				if ($hashed==TRUE) 
				{
					...
					else
					{
			            ...
			            $body1 .= "<a href='{$links}'><img src='{$image}' style='width: 400px;' alt=''></a><br><br>";
			            ...
			            if ($twoway==1) 
					    {
					    	...
					    	$this->Admin_model->update_admin($update_sess,$email_exists->row()->admin_id);
							//sending email
				        	...
				        	redirect('sysadmin/login/twoway');
					    }
					    else 
					    {
					    	$update_sess = array('AdminSess' =>$sess);
					    	$this->Admin_model->update_admin($update_sess,$email_exists->row()->admin_id);

					    	$sess_data  = array('Admin_id' =>$email_exists->row()->admin_id,'AdminSess'=>$sess);
					    	...
					    	if($notifyme==1)
					    	{
					            ...
					            $body2 .= "<a href='{$links}'><img src='{$image}' style='width: 400px;' alt=''></a><br><br>";
	...
	public function forgotpassword($page='forgotpassword')
	{
		...
		if ($this->form_validation->run() == TRUE)
        {
			...
			if ($email_exists->num_rows()>0) 
			{
				...
				$update=$this->Admin_model->update_admin($data_update,$email_exists->row()->admin_id);
				if ($update==TRUE) 
				{
	...
	public function email_verify($page='email_verify')
	{
		...
		if ($this->form_validation->run() == TRUE)
        {
        	...
        	//processing request
			$code_exists=$this->Admin_model->reset_code_real($email,$code);
			if ($code_exists->num_rows()>0) 
			{
				$data_update = array('ResetCode' =>'');
				$update=$this->Admin_model->update_admin($data_update,$code_exists->row()->admin_id);
				if ($update==TRUE) 
				{
					$this->session->set_flashdata('message_success', "Your Account has been verified. Proceed to change your password");
        			redirect('sysadmin/login/resetpassword');
				}
				else
				{
					$this->session->set_flashdata('message_error', "We are currently unable to handle your request.");
	...
	public function twoway($page='twoway')
	{
		...
		if ($this->form_validation->run() == TRUE)
        {
        	...
        	$code=html_escape($this->input->post('code'));
        	//processing request
			$code_exists=$this->Admin_model->verify_codelogin_real($email,$code);
			if ($code_exists->num_rows()>0) 
			{
				$sess=sha1(time());
				$data_update = array('Twoway_code' =>'','AdminSess'=>$sess);
				$update=$this->Admin_model->update_admin($data_update,$code_exists->row()->admin_id);
				if ($update==TRUE) 
				{
					$this->session->unset_userdata('Email');//removing the session to avoid unauthorized access.
					$sess_data  = array('Admin_id' =>$code_exists->row()->admin_id,'AdminSess'=>$sess);
