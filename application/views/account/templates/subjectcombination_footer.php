<!-- BEGIN: Footer-->
    ...
    <script>
        ...
        $('#edit_schooltype').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-title').text('Add Subject Combination')
            if(recipient) 
            {
                $.ajax(
                {
                    url: '<?php echo base_url();?>account/subjects/get_subject_school/'+recipient,
                    type: "GET",
                    dataType: "json",
                    success:function(data) 
                    {
                        ...
                        $.each(data, function(key, value) {
                            ...
                            $('input[name="load"]').val(value.Unit_load);
                            $('input[name="code"]').val(value.Subject_code);
...
<script type="text/javascript">
        $('#addschooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Assign Subject to CLass')
            ...
            $('select[name="school"]').change(function(){
              ...
              school=$(this).val();
               $.ajax(
                {
                    url: '<?php echo base_url();?>account/subject_combination/get_subject_school/'+school,
                    type: "GET",
                    dataType: "json",
                    success:function(data) 
                    {
                        ...
                        //$('select[name="products"]').append('<option value=" ">Select Product</option>');
                        $.each(data, function(key, value) {
                             $('select[name="combination[]"]').append('<option value="'+ value.Subject_id +'">'+ value.Subject_name +'</option>');
                        });
                    }
                });
            });
...
<script type="text/javascript">
  $(document).ready(function() {
    var namefield='<div class="col-sm-4 form-group mb-4"><input class="form-control" type="text" placeholder="Subject Name" name="name[]"></div>';
    var codefield='<div class="col-sm-4 form-group mb-4"><input class="form-control" type="text" placeholder="Subject Code" name="code[]"></div>';
    var loadfield='<div class="col-sm-4 form-group mb-4"><input class="form-control" type="text" placeholder="Unit Load" name="load[]"></div>';
    $('#add_column').on('click', function() {
      //$('select[name="products"]').append('<option value=" ">Select Product</option>');
      $('#form').append(namefield);
      $('#form').append(codefield);
      $('#form').append(loadfield);
    });
  });
</script>
