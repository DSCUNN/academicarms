<!-- BEGIN: Footer-->
    ...
    <script type="text/javascript">
        $('#addschooltype').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-title').text('Add Result Pin')
            if(recipient) 
            {
                $.ajax(
                {
                    url: '<?php echo base_url();?>account/result_pins/get_package_id/'+recipient,
                    type: "GET",
                    dataType: "json",
                    success:function(data) 
                    {
                        ...
                        $.each(data, function(key, value) {
                            //getting the amount to be paid by the organizer
                            price=value.New_price;
                            $('input[name="pin_number"]').keyup(function(){
                                qty=$(this).val();
    ...
    <script type="text/javascript">
        $('#edit_schooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Edit Pins')
            if(recipient) 
            {
                $.ajax(
                {
                    url: '<?php echo base_url();?>account/result_pins/get_pin_id/'+recipient,
                    type: "GET",
                    dataType: "json",
                    success:function(data) 
                    {
                        ...
                        $.each(data, function(key, value) {
                            stat=value.Status;
                            $('input[name="name"]').val(value.Pin_number);
                            $('input[name="code"]').val(value.Serial_number);
                            $('input[name="id"]').val(value.Pin_id);
                        });
                    }
                });
            }
    ...
    <script type="text/javascript">
        $('#delete_schooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Delete Class')
