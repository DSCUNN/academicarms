<!-- BEGIN: Footer-->
    ...
    <script type="text/javascript">
        $('#addschooltype').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-title').text('Add new School Type')
    ...
    <script type="text/javascript">
        $('#edit_schooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Edit School Type')
            if(recipient) 
            {
                $.ajax(
                {
                    url: '<?php echo base_url();?>account/school_type/get_schooltype_id/'+recipient,
                    type: "GET",
                    dataType: "json",
                    success:function(data) 
                    {
                        ...
                        $.each(data, function(key, value) {
                            stat=value.Status;
                            $('input[name="name"]').val(value.School_type_name);
                            $('input[name="id"]').val(value.School_type_id);
                        });
                    }
                });
            }
    ...
    <script type="text/javascript">
        $('#delete_schooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Delete School Type')
            $('input[name="id"]').val(recipient);
        })
    </script>
