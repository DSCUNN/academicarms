<!-- BEGIN: Footer-->
    ...
    <script type="text/javascript">
        $('#addschooltype').on('show.bs.modal', function (event) {
            ...
            var modal = $(this)
            modal.find('.modal-title').text('Add new Academic Session')
    ...
    <script type="text/javascript">
        $('#edit_schooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Edit Academic Session')
            if(recipient) 
            {
                $.ajax(
                {
                    url: '<?php echo base_url();?>account/academic_sessions/get_session_id/'+recipient,
                    type: "GET",
                    dataType: "json",
    ...
    <script type="text/javascript">
        $('#delete_schooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Delete Academic Session')
            $('input[name="id"]').val(recipient);
        })
    </script>
<script type="text/javascript">
  $(document).ready(function() {
    var namefield='<div class="col-sm-12 form-group mb-4"><input class="form-control" type="text" placeholder="Session" name="name[]"></div>';
