<!-- BEGIN: Footer-->
        ...
        $('#addschooltype').on('show.bs.modal', function (event) {
            ...
            $('select[name="school"]').change(function(){
              ...
              school=$(this).val();
               $.ajax(
                {
                    url: '<?php echo base_url();?>teachers/students/get_class_school/'+school,
                    type: "GET",
                    dataType: "json",
        ...
        $('#edit_schooltype').on('show.bs.modal', function (event) {
            ...
            $('select[name="school"]').change(function(){
              ...
              school=$(this).val();
               $.ajax(
                {
                    url: '<?php echo base_url();?>teachers/students/get_class_schools/'+school,
                    type: "GET",
                    dataType: "json",
                    success:function(data) 
                    {
            ...
            if(recipient) 
            {
                $.ajax(
                {
                    url: '<?php echo base_url();?>teachers/students/get_student_id/'+recipient,
                    type: "GET",
                    dataType: "json",
                    success:function(data) 
                    {
                        ...
                        $.each(data, function(key, value) {
                            ...
                            $('input[name="username"]').val(value.Reg_no);
                            $('input[name="id"]').val(value.Student_id);
                        });
                    }
                });
            }
    ...
    <script type="text/javascript">
        $('#delete_schooltype').on('show.bs.modal', function (event) {
            ...
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this)
            modal.find('.modal-title').text('Delete Teacher')
            $('input[name="id"]').val(recipient);
        })
    </script>
<script type="text/javascript">
  $(document).ready(function() {
    var namefield='<div class="col-sm-6 form-group mb-4"><input class="form-control" type="text" placeholder="Student Name" name="name[]"></div>';
    var usernamefield='<div class="col-sm-6 form-group mb-4"><input class="form-control" type="text" placeholder="Reg Number" name="username[]"></div>';
