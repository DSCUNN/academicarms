...
{ 
    ...
    // Function to write the Paystack Keys to the constant file
    function Paystack($path = null, $data = []) 
    {
        if (file_exists($path['constant_path'])) {
            // Open the file
            $constant_file = file_get_contents($path['constant_path']);

            //set new database configuration
            $new  = str_replace("{PAYSTACK_PUB_KEY}",$data['paystack_public'],$constant_file);
            $new  = str_replace("{PAYSTACK_PRI_KEY}",$data['paystack_private'],$new);

            // Write the new database.php file
            $handle = fopen($path['constant_output_path'],'w+');

            // Chmod the file, in case the user forgot
            @chmod($path['constant_output_path'],0777);

            // Verify file permissions
            if (is_writable($path['constant_output_path'])) {
                // Write the file
                if (fwrite($handle,$new)) {
                    return true;
                } else {
    ...
    // Function to write the Flutterwave config file
    function Flutterwave($path = null, $data = []) 
    {
        if (file_exists($path['flutterwave_path'])) {
            // Open the file
            $flutterwave_file = file_get_contents($path['flutterwave_path']);

            //set new database configuration
            $new  = str_replace("{ENVIRONMENT}",$data['environ'],$flutterwave_file);
            $new  = str_replace("{FLUTTER_PUB_KEY}",$data['flutter_public'],$new);
            $new  = str_replace("{FLUTTER_SEC_KEY}",$data['flutter_private'],$new);

            // Write the new database.php file
            $handle = fopen($path['flutterwave_output_path'],'w+');

            // Chmod the file, in case the user forgot
            @chmod($path['flutterwave_output_path'],0777);

            // Verify file permissions
            if (is_writable($path['flutterwave_output_path'])) {
                // Write the file
                if (fwrite($handle,$new)) {
                    return true;
                } else {
